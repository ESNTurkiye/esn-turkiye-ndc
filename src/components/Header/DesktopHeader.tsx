            ...
            <div className={`container mx-auto px-4 sm:px-6 md:px-10 lg:px-16 xl:px-24 flex justify-between items-center transition-all duration-300 ease-in-out ${
                ...
                >
                    ...
                    <div className="hidden lg:flex flex-col border-l-2 border-white/30 pl-2 md:pl-3 leading-tight transition-all duration-300 ease-in-out">
                        ...
                        <span className={`text-white/80 font-body text-xs font-normal tracking-normal mt-1 transition-all duration-300 ease-in-out ${
                ...
                <nav className="hidden md:block">
                    <ul className={`flex items-center space-x-8 text-white font-body font-bold tracking-wider transition-all duration-300 ease-in-out ${
                        ...
                        <li onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })} className="relative cursor-pointer py-4 group">
                            <span className="relative z-10">HOME</span>
                            <span className="absolute bottom-2 left-0 right-0 h-1 bg-esn-cyan origin-center scale-x-0 transition-transform duration-300 ease-in-out group-hover:scale-x-100"></span>
                        </li>
                        
                        <li className="relative group py-4">
                            <button className="flex items-center gap-1 cursor-pointer uppercase relative z-10">
                                About
                                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" /></svg>
                            </button>
                            
                            <div className="absolute top-full left-0 w-48 bg-white shadow-xl rounded-b-lg border-t-2 border-esn-cyan opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300 transform translate-y-2 group-hover:translate-y-0 z-50">
                                <ul className="py-2 text-gray-700 font-normal normal-case">
                                    <li onClick={() => scrollToSection('introduction')} className="px-4 py-2 hover:bg-gray-50 hover:text-esn-cyan cursor-pointer border-b border-gray-100 transition-colors">Introduction</li>
                                    <li onClick={() => scrollToSection('report-2025')} className="px-4 py-2 hover:bg-gray-50 hover:text-esn-cyan cursor-pointer border-b border-gray-100 transition-colors">2025 Edition</li>
                                    <li onClick={() => scrollToSection('team')} className="px-4 py-2 hover:bg-gray-50 hover:text-esn-cyan cursor-pointer transition-colors">The Team</li>
                                </ul>
                            </div>
                        </li>
